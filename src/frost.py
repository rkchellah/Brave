...
class Frost:
    ...
    def __init__(self, config: dict):
        self.config = config
        logging.info("✅ Frost loaded — Forex Fury style mean reversion")
        logging.info(f"   Session: Asian only (00:00–06:00 UTC)")
        logging.info(f"   Entry: {self.MIN_DEVIATION_PIPS}–{self.MAX_DEVIATION_PIPS} pip deviation from {self.MA_PERIOD}-MA")
        logging.info(f"   Max spread: {self.MAX_SPREAD_PIPS} pips")
        logging.info(f"   Pairs: GBPUSD, USDCAD, EURCHF")
    ...
    def analyze(self, symbol: str, provided_rates: dict | None = None) -> dict | None:
        ...
        if provided_rates is None:
            if not self._is_asian_session():
                logging.info(f"   [{symbol}] Frost: Outside Asian session — skipping")
                return None
            if not self._is_safe_entry_time():
                logging.info(f"   [{symbol}] Frost: Past cutoff time (05:30 UTC) — no new entries")
                return None
        ...
        if not candles or len(candles) < self.MA_PERIOD + self.ATR_PERIOD:
            logging.error(f"   [{symbol}] Frost: Insufficient M15 data")
            return None
        ...
        if provided_rates is None:
            ...
            if spread_pips > self.MAX_SPREAD_PIPS:
                ...
                return None
            logging.info(f"   [{symbol}] Frost: Spread OK ({spread_pips:.1f} pips)")
        ...
        logging.info(f"   [{symbol}] Frost: ATR = {atr_pips:.1f} pips ✅")
        ...
        logging.info(f"   [{symbol}] Frost: Deviation {abs_deviation:.1f} pips ✅")

        # ── 8. Confirm ranging market (no clear trend) ───────────
        if self._is_trending(candles, point):
            logging.info(f"   [{symbol}] Frost: Market is trending — mean reversion invalid")
            return None

        # ── 9. Generate mean-reversion signal ────────────────────
        direction = "SELL" if deviation_pips > 0 else "BUY"
        return self._build_signal(symbol, current_price, ma_value, deviation_pips, point, direction)
    ...
    def _get_spread_pips(self, symbol: str, point: float) -> float | None:
        ...
        if tick is None:
            logging.warning(f"   [{symbol}] Frost: Could not get tick data")
            return None
